...
#out_ori = cv2.VideoWriter('liveVid/'+file+'_livingRoom.mp4', fourcc, 21, (dispW, dispH))
#out_transformed = cv2.VideoWriter('liveVid/'+file+'_livingRoom_tranformed.mp4', fourcc, 21, (dispW, dispH))
...
regions = [
    (0, 0), # Top-left corner (1,1)
    (region_width, 0), # Top-right corner (1,2)
    (0, region_height), # Bottom-left corner (2,1)
    (region_width, region_height) # Bottom-right corner (2,2)
    ]
region_map = {0: 'D', 1: 'C', 2: 'A', 3: 'B'}
...
def run():
    ...
    #cap = cv2.VideoCapture('vidPengujian/baringLantai_0_Nando.mp4')
    cap = cv2.VideoCapture(0)
    
    # Give weight files to the model and load the network
    net = cv2.dnn.readNet(model='models/livingRoom_120_best.weights', config='utils/yolov4-tiny-test.cfg')
